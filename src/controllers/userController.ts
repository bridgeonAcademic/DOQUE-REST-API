...
export const getUserById = async (req: CustomRequest, res: Response) => {
  ...
  res
    .status(200)
    .json(new StandardResponse("Profile fetched successfully", user));
};
...
export const getAllUsers = async (req: Request, res: Response) => {
  ...
  res
    .status(200)
    .json(new StandardResponse("User Updated successfully", users));
};
    ...
    res
      ...
      .json(
        new StandardResponse(
          "Tasks assigned to the user retrieved successfully",
          tasksWithDetails
        )
      );
  } catch (error) {
