...
export const getTaskById = async (req: CustomRequest, res: Response) => {
	...
	res.status(200).json(new StandardResponse("Task retrieved succesfully", task, 200));
};

export const updateTask = async (req: CustomRequest, res: Response) => {
	const { taskId } = req.params;
	const { title, description, dueDate, priority, assignedTo, listId } = req.body;
...
export const moveTask = async (req: CustomRequest, res: Response) => {
	...
	const task = await Task.findByIdAndUpdate(taskId, { listId: targetListId }, { new: true });
